...
export function dialogWrapper(str: any) {
...
export function vueTemplate(str: any) {
  return `<template>
    <div>
      ${str}
    </div>
...
export function vueScript(str: any) {
  return `<script>
    ${str}
  </script>`
}

export function cssStyle(cssStr: any) {
  return `<style>
    ${cssStr}
  </style>`
}

function buildFormTemplate(scheme: any, child: any, type: any) {
  let labelPosition = ''
...
// span不为24的用el-col包裹
function colWrapper(scheme: any, str: any) {
 ...
 */
export function makeUpHtml(formConfig: any, type: any) {
  const htmlList: Array<any> = []
  confGlobal = formConfig
  // 判断布局是否都沾满了24个栅格，以备后续简化代码结构
  someSpanIsNot24 = formConfig.fields.some((item: any) => item.__config__.span !== 24)
  // 遍历渲染每个组件成html
